...
def dlocal(command):
    """
    Execute the `command` (str) on the remote docker host `env.DOCKER_HOST`.
...
@task
def get_files_from_docker_host_dir(remotepath, localpath=None):
    """
    Download `remotepath` from docker host to `localpath` on this computer.
    """
    pass
    # if format == 'html':
